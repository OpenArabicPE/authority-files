...
<xsl:stylesheet exclude-result-prefixes="#all" version="3.0" xmlns="http://www.tei-c.org/ns/1.0" xmlns:bgn="http://bibliograph.net/" xmlns:dc="http://purl.org/dc/elements/1.1/"
    xmlns:genont="http://www.w3.org/2006/gen/ont#" xmlns:oape="https://openarabicpe.github.io/ns" xmlns:opf="http://www.idpf.org/2007/opf" xmlns:pto="http://www.productontology.org/id/"
    xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:re="http://oclcsrw.google.code/redirect" xmlns:schema="http://schema.org/" xmlns:srw="http://www.loc.gov/zing/srw/"
    xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:umbel="http://umbel.org/umbel#" xmlns:viaf="http://viaf.org/viaf/terms#" xmlns:xd="http://www.pnp-software.com/XSLTdoc"
    xmlns:xi="http://www.w3.org/2001/XInclude" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <xsl:if test="not(doc-available($p_url-bibliography))">
            <xsl:message terminate="yes">
                <xsl:text>The specified authority file has not been found at </xsl:text>
    ...
    <xsl:template match="tei:item" mode="m_debug" priority="10">
        <xsl:copy>
    ...
    <xsl:template mode="m_debug" match="node() | @*">
        <xsl:copy>
            <xsl:apply-templates select="@* |node()" mode="m_debug"/>
        </xsl:copy>
    </xsl:template>
    
    <xsl:template match="tei:biblStruct[tei:monogr/tei:title[@ref]]" mode="m_debug" priority="10">
        <xsl:variable name="v_authority" select="oape:get-entity-from-authority-file(descendant::tei:title[@ref], $p_local-authority, $v_bibliography)"/>
        <xsl:copy-of select="oape:merge-nodes(., $v_authority, 'target')"/>
    </xsl:template>
    <!--<xsl:template match="node()[following-sibling::node()[local-name() = current()/local-name()]]" mode="m_debug" priority="9">
        <xsl:copy-of select="oape:merge-nodes(., following-sibling::node()[local-name() = current()/local-name()][1], 'source')"/>
    </xsl:template>-->
    ...
    <xsl:template match="tei:orgName[matches(., '^[A-Z]{2}\-')]" mode="m_debug">
        ...
        <xsl:copy>
            <xsl:apply-templates mode="m_identity-transform" select="@*"/>
            <xsl:apply-templates mode="m_identity-transform" select="$v_org/descendant::tei:placeName[1]"/>
            <xsl:text>, </xsl:text>
            <xsl:apply-templates mode="m_identity-transform" select="$v_org/descendant::tei:orgName[1]"/>
        </xsl:copy>
    </xsl:template>
    <xsl:template match="tei:orgName" mode="m_debug">
        <item>
            <ab>
                <xsl:copy-of select="oape:get-entity-from-authority-file(., $p_local-authority, $v_organizationography)"/>
            </ab>
        </item>
    </xsl:template>
    ...
    <xsl:template match="tei:placeName" mode="m_debug">
        ...
        <item>
            <ab>
                <xsl:value-of select="oape:query-gazetteer(., $v_gazetteer, $p_local-authority, 'id', '')"/>
            </ab>
        </item>
    </xsl:template>
        ...
        <xsl:variable name="v_biblStruct">
            <xsl:choose>
                <xsl:when test="$v_self/ancestor::tei:biblStruct">
                    <xsl:apply-templates mode="m_identity-transform" select="./ancestor::tei:biblStruct[1]"/>
                </xsl:when>
