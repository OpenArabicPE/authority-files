...
<xsl:stylesheet exclude-result-prefixes="#all" version="3.0"
    ...
    <!-- PROBLEMS: 
         - If run on names in Latin alphabet and with $p_add-mark-up-to-input set to "true()", symbols such as -,. and ' are stripped out
    -->
    ...
    <xsl:template match="tei:persName" priority="30">
        <xsl:variable name="v_self-linked" select="oape:link-persname-to-authority-file(., $p_local-authority, $v_personography, $p_add-mark-up-to-input)"/>
        <xsl:choose>
...
<!--                <xsl:variable name="v_no-rolename-flat" select="oape:string-remove-spaces(oape:string-normalise-characters($v_no-rolename))"/>-->
                ...
                <xsl:variable name="v_name-marked-up-linked" select="oape:link-persname-to-authority-file($v_no-rolename/tei:persName, $p_local-authority, $v_personography, $p_add-mark-up-to-input)"/>
                    <xsl:choose>
                    <!-- test if a match was found in the authority file -->
