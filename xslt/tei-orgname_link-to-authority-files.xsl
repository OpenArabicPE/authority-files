...
<xsl:stylesheet  
    ...
    <xsl:param name="p_url-authority-file" select="'../data/tei/personography_OpenArabicPE.TEIP5.xml'"/>
<!--    <xsl:param name="p_url-master" select="'/BachUni/BachBibliothek/GitHub/ProjectJaraid/jaraid_source/authority-files/jaraid_authority-file.TEIP5.xml'"></xsl:param>-->
   ...
   <!-- Identity transformation -->
    ...
    <xsl:template match="tei:orgName" priority="10">
        <xsl:variable name="v_corresponding-org" select="oape:get-entity-from-authority-file(., $p_local-authority, $v_file-entities-master)"/>
        <xsl:choose>
            <!-- fallback: name is not found in the authority file -->
            <xsl:when test="$v_corresponding-org = 'false()'">
    ...
    <xsl:template match="tei:revisionDesc" name="t_9">
        ...
        <xsl:copy>
            ...
            <xsl:element name="tei:change">
                ...
                <tei:ref target="{$p_url-authority-file}">
                    <xsl:value-of select="$p_url-authority-file"/>
                </tei:ref>
                <xsl:text>)  to </xsl:text><tei:gi>orgName</tei:gi><xsl:text>s without such references based on  </xsl:text><tei:gi>org</tei:gi><xsl:text>s mentioned in the authority file.</xsl:text>
            </xsl:element>
            <xsl:apply-templates select="node()"/>
        </xsl:copy>
    </xsl:template>
